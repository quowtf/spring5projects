...
import quo.wtf.worldgdp.mapper.CityRowMapper;
import quo.wtf.worldgdp.model.City;
...
public class CityDAO {
	...
	public List<City> getCities(String countryCode, Integer pageNo) {
		...
		return namedParamJdbcTemplate.query("SELECT " + " id, name, countrycode country_code, district, population "
				+ " FROM city WHERE countrycode = :code" + " ORDER BY Population DESC"
				+ ((pageNo != null) ? " LIMIT :offset , :size " : ""), params, new CityRowMapper());
	}
	...
	public City getCityDetail(Long cityId) {
		...
		return namedParamJdbcTemplate.queryForObject("SELECT id, " + " name, countrycode country_code, "
				+ " district, population " + " FROM city WHERE id = :id", params, new CityRowMapper());
	}
	...
	public Long addCity(String countryCode, City city) {
		...
		namedParamJdbcTemplate.update("INSERT INTO city(" + " name, countrycode, " + " district, population) "
